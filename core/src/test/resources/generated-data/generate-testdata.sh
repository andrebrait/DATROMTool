...
for i in $(seq 1 7); do
	...
	printf "\x4E\x45\x53\x1A$(base64 /dev/zero | head -c 12)" | cat - "${c}.txt" > "headered-${c}.txt"
done

base64 /dev/urandom | head -c "10000001" > "10000001.txt"
printf "\x4E\x45\x53\x1A$(base64 /dev/zero | head -c 12)" | cat - "10000001.txt" > "headered-10000001.txt"
